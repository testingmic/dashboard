...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateRealTimeData() {
        fetch(baseUrl + 'dashboard/getRealTimeData')
...
function initializeCharts() {
    ...
    if (revenueChartCanvas && typeof arrayStream.revenueChart !== 'undefined') {
        const ctx = revenueChartCanvas.getContext('2d');
        new Chart(ctx, {
            type: 'line',
            data: {
                labels: arrayStream.revenueChart?.map(item => item.date),
                datasets: [{
                    label: 'Revenue',
                    data: arrayStream.revenueChart?.map(item => item.daily_revenue ?? 0),
    ...
    if (orderStatusCanvas && typeof orderStats !== 'undefined') {
